...
namespace DocTracker.Controllers {
   ...
   public class TrackDocEventsController : ControllerBase {
      ...
      */
      ...
      /// <summary>
      /// Insert one or more document operation in the database
      /// </summary>
      /// <remarks>
      /// Don't fill these fields in the request body: partitionKey, rowKey, timestamp, eTag <br /> 
      /// these fields are auto-generated by the application. In the next version we'll remove them from UI, sorry. <br />
      /// <br/>
      /// The upload event for a certain document is unique, subsequent attempts to insert the upload for the same document will be skipped.<br/>
      /// Processing events duplicates are not checked, if you insert the same processing event multiple times you will have duplicate events in the database.<br/>
      /// <br/>
      /// Some fields in the request body are not always needed: field names beginning with "upload" are only needed for upload events, 
      /// fields beginning with "processing" are only needed for processing events<br/>
      /// <br/>
      /// See this <a href="https://github.com/antotest-spec/DocTracker/blob/master/DocTracker/Test%20Example.json">template</a> 
      /// to get an idea of how to format the request body in an optimal way. You are encouraged to change fields values in the template to create your own tests.
      /// </remarks>
